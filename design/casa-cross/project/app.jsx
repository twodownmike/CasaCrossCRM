...
function App() {
  ...
  // Hide app header on detail screens that have their own
  ...
  return (
    <div className="phone-frame">
      ...
      <div className="scroll" key={route.view + (route.id || "")}>
        {body}
      </div>

      {showFab && (
        <button className="fab" aria-label="New" onClick={() => setCreateOpen(true)}>
          <I.plus />
        </button>
      )}
      ...
      <Sheet open={createOpen} onClose={() => setCreateOpen(false)} title="Create new">
        <div style={{ paddingTop: 8, display: "flex", flexDirection: "column", gap: 8 }}>
          {[
            ...
            const Ico = it.icon;
            return (
              <button key={i} className="card-row" style={{ borderRadius: 12, border: "1px solid var(--hair)", borderBottom: "1px solid var(--hair)" }}
